...
Schema fixtures are validated by `pnpm validate:schemas`. Policy proof points are validated by `pnpm validate:policy`. The sample policy repository is validated by `pnpm validate:sample-policy`. The examples in this directory are documentation examples and should remain consistent with OpenAPI, CLI, and schema contracts.
