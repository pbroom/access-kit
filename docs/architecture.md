...
7. Audit and evidence plane: append-only event model, hash-chain integrity reports, validation evidence, control mapping, ConMon metrics, POA&M inputs, SIEM metadata, and export contracts.
...
- ATO evidence exports include source events, audit integrity status, control mappings, ConMon metrics, POA&M inputs, artifact metadata, and SIEM export metadata.
