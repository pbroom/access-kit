...
The core package also defines persistent graph, audit, and job repository contracts plus a persistence-readiness report for future production stores. The local JSON graph adapter persists graph facts for development and validation, but the current API runtime still treats configured local JSON and file repositories as proof points; production deployment must wire durable adapters before live connector writes.
