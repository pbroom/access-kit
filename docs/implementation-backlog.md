...
| AK-013 | Runtime contract enforcement and API parity | ready | P0 | AK-009 | no | contract-runtime | codex/rebac-runtime-contract-hardening | - | Runtime handlers use schema-backed decoders, reject invalid payloads, and a parity smoke covers OpenAPI, route, and CLI operation drift. | Malformed admin and runtime requests cannot bypass schema constraints or undocumented endpoint gaps. | Start before durable storage runtime to stop contract drift. |
| AK-014 | Runtime persistence boundary consolidation | ready | P0 | AK-013 | no | storage-runtime | codex/rebac-runtime-persistence-boundary | - | API runtime has one canonical persistence boundary, runtime audit uses the append-only adapter, and disk readers decode or migrate persisted shape explicitly. | Audit integrity, persisted state, and local proof-point failure modes are explicit instead of silently falling back to weaker storage. | Complete before AK-010 durable storage implementation. |
