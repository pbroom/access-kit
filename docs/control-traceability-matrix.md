...
| AU-6 Audit Review | Audit integrity, signed audit windows, SIEM-ready export metadata, delivery failure findings, and replay receipts. | `schemas/audit-integrity.schema.json`, `schemas/audit-export.schema.json`, `tests/core/production-audit.test.ts` | Approved SIEM forwarding deployment and alert playbooks. |
