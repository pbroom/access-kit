...
| AC-6 Least Privilege | Connector capability model, read-only discovery, synthetic enforcement gates. | [Connector Contract](connector-contract.md), `schemas/enforcement-readiness.schema.json` | Live connector least-privilege reviews. |
| AU-2/AU-3 Audit Events | Append-only audit event schema and event coverage. | [Audit Event Model](audit-event-model.md), `schemas/audit-event.schema.json` | Production retention and centralized monitoring. |
| AU-6 Audit Review | Audit integrity and SIEM-ready export metadata. | `schemas/audit-integrity.schema.json`, `schemas/audit-export.schema.json` | Approved SIEM forwarding and alert playbooks. |
...
| IA | Authentication boundary is documented and delegated to approved IdPs. | [System Context and Boundary](system-context-and-boundary.md), [Security Model](security-model.md) | Production IdP configuration evidence. |
| IR | Emergency revocation, connector outage, compromised credential, and decision API outage runbooks. | `runbooks/*.md` | Exercised incident records and post-action reviews. |
