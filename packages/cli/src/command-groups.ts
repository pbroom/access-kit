...
import { renderShellCompletion } from "./completions.js";
import { writeCliError } from "./errors.js";
import { normalizeEmergencyRevokeForIdempotency, relationshipId } from "./idempotency.js";
import {
  buildEmergencyRevokePayload,
  buildProvisioningExecutionPayload,
  buildProvisioningJobPayload,
  readEvidencePackageFile
} from "./payload-builders.js";
import { resolveRuntimeOptions, type CliContext } from "./runtime-options.js";

export function addCommandGroups(program: Command, context: CliContext): void {
...
function addEmergencyCommands(program: Command, context: CliContext): void {
  ...
  revoke.action(withApi(context, revoke, (client, args) => {
    ...
    const requestBody = buildEmergencyRevokePayload(options, context, readString(args, 0, "grant-id"));
