...
describe("generated policy test artifacts", () => {
  it("builds compact starter authorization tests and temp materialized derivatives from each sample model", async () => {
    ...
    expect(v3Suite?.reviewNotice).toBe(GENERATED_POLICY_REVIEW_NOTICE);
    expect(v3Suite?.tupleFixture.reviewOnly).toBe(true);
    expect(v3Suite?.tupleFixture.subjects.map((subject) => subject.sourceSystem)).toEqual(
      expect.arrayContaining(["generated-policy-tests"])
    );
    expect(v3Suite?.authorizationTests.every((test) => test.reviewOnly)).toBe(true);
    expect(v3Suite?.authorizationTests.map((test) => test.category)).toEqual(
    ...
    expect(v3Suite?.authorizationTests.map((test) => test.expected.reasonCode)).toEqual(
      expect.arrayContaining([
  ...
  it("labels migration regression snapshots as review aids that do not replace authored coverage", async () => {
    const generated = await generatePolicyTestArtifacts({ root: process.cwd() });
    const migrationSnapshot = generated.migrationSnapshots.find((snapshot) =>
      snapshot.migration.fromVersion === "policy:case-docs-v2"
        && snapshot.migration.toVersion === "policy:case-docs-v3"
    );
    ...
    expect(migrationSnapshot?.migration).toMatchObject({
      fromVersion: "policy:case-docs-v2",
      toVersion: "policy:case-docs-v3"
    });
    expect(migrationSnapshot?.targetModel.contextConstraints).toEqual(
      expect.arrayContaining(["accessTime", "deviceTrustLevel", "riskScore"])
    );
    expect(migrationSnapshot?.targetModel.caveats).toEqual(
      expect.arrayContaining(["managed_low_risk_read"])
    );
    expect(migrationSnapshot?.reviewerChecklist.join(" ")).toContain("fail-closed policy inputs");
